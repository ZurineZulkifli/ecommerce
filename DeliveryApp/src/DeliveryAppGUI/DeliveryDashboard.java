...
public class DeliveryDashboard extends JFrame {
    ...
    public DeliveryDashboard(String token, int userId) {
        ...
        scrollPane.setBounds(50, 100, 680, 300);
        getContentPane().add(scrollPane);

        JButton refreshBtn = new JButton("Refresh");
        refreshBtn.setFont(new Font("Consolas", Font.PLAIN, 18));
        refreshBtn.setBounds(100, 420, 160, 40);
    ...
    private void loadDeliveries() {
        try {
            JSONObject response = ApiClient.getJsonWithAuth("/deliveries/assigned.php", token);
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                ...
                if (response.getBoolean("success")) {
                    ...
                    loadDeliveries(); //Refresh table after success
                } else {
